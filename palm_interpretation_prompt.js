...
- 反之，要**充分用足**数据里现成的可讲维度：每条线的 \`depthGradient\`（起止深浅走势，如"起点深、末端渐浅"）、生命线的 \`lifeTimeline\`（流年分段与断口年龄区间，如"XX–XX 岁有一处转折"）。这些是你唯一能讲具体的依据，**务必在对应线的段落里点出来**（例如讲生命线时带一句"这条线约 XX–XX 岁有一处转折/整体走势如何"；讲智慧线时带一句"起端深、往末端渐渐变浅"），不要空泛带过、也不要只报字段名。
